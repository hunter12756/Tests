...
class Enemy(pygame.sprite.Sprite):
    ...
    def collision(self,orb_group,user): #if the bullet touches the enemies they both disappear(orb destroys the enemy)
        for projectile in orb_group:
...
class Orb(pygame.sprite.Sprite): #makes orbs
    def __init__(self, pos, direction, angle): #uses position and direction of the Player sprite
        ...
        self.image.fill(yellow) #orbs are yellow
    ...
    def update(self): #moves bullet
        ...
        #deletes the orbs if they go off the screen
...
def main(): #sets up screen
    ...
    user = User() #adding pygame.sprite.Sprite breaks it, THE WHOLE PLAYER CLASS IS STR8 BOONED
    ...
    while x == True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if ( event.key == pygame.K_w or event.key == pygame.K_UP ) and user.speed < speedCap:
                    pygame.key.set_repeat(2,1) #prevents superspeed
                    user.speed += 1
                if ( event.key == pygame.K_s  or event.key == pygame.K_DOWN ) and user.speed > speedMin:
